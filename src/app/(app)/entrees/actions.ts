...
import { type LengthUnit, lengthToMeters } from "@/utils/length";
...
function readEntreeFormData(formData: FormData) {
  ...
  const longueurUnit = String(formData.get("longueurUnit")) as LengthUnit;
  const largeurValue = Number(formData.get("largeurValue"));
  const largeurUnit = String(formData.get("largeurUnit")) as LengthUnit;
